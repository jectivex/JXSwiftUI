...
final class TapGestureModifierTests: JXSwiftUITestsBase {
    func testValid() throws {
        var jxValue = try context.eval("swiftui.EmptyView().onTapGesture(() => {})")
        var tap = try TapGestureModifier(jxValue: jxValue)
        let _ = tap.view(errorHandler: errorHandler)
        
        jxValue = try context.eval("swiftui.EmptyView().onTapGesture(2, () => {})")
    ...
    func testInvalid() throws {
        expectingError {
            let jxValue = try context.eval("swiftui.EmptyView().onTapGesture(2)")
            let tap = try TapGestureModifier(jxValue: jxValue)
            let _ = tap.view(errorHandler: errorHandler)
        }
        expectingError {
            let jxValue = try context.eval("swiftui.EmptyView().onTapGesture(2, 'string')")
            let tap = try TapGestureModifier(jxValue: jxValue)
            let _ = tap.view(errorHandler: errorHandler)
        }
    }
